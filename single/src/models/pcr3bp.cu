...
__device__ static void fPcr3bp(float* f, float* x, float* dx);
__device__ static void zPcr3bp(float* h, float* x, float* dx);
__device__ static float jPcr3bp(float* x);
...
__device__ static const float coef[] = {1.901109735892602E-07}; // PCR3BP trajectory attributes (mu)
 ...
 */
__device__ static void fPcr3bp(float* f, float* x, float* dx){    
    float r1 = pow(pow(x[0]+coef[0],2)+pow(x[1],2),1.5);
    float r2 = pow(pow(x[0]-1+coef[0],2)+pow(x[1],2),1.5);
    f[0] = x[2];
    f[1] = x[3];
 ...
 */
__device__ static float jPcr3bp(float* x){    
    float r1 = pow(pow(x[0]+coef[0],2)+pow(x[1],2), 0.5);
    float r2 = pow(pow(x[0]-1+coef[0],2)+pow(x[1],2), 0.5);
    float J = pow(x[0], 2.0) + pow(x[1], 2.0) + (2*(1-coef[0])/r1) + (2*coef[0]/r2) + coef[0]*(1 - coef[0]) - (pow(x[2], 2.0) + pow(x[3], 2.0));
    return J;
}
