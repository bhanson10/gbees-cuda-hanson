...
static __device__ float gaussProbability(int32_t* key, GridDefinition* gridDefinition, Measurement* measurements);

/** Calculate gaussian probability at state x given mean and covariance (used for update measurement) */
static __device__ float gaussProbability(float* y, Measurement* measurement);
...
static __device__ void initializeGridBoundary(int offset, int iterations, float* localArray, GridDefinition* gridDefinition, Global* global);

/** Normalize probability distribution */
static __device__ void normalizeDistribution(int offset, int iterations, float* localArray, float* globalArray, Grid* grid);

/** Compute grid bounds */
static __device__ void gridBounds(float* output, float* localArray, float* globalArray, float boundaryValue, float(*fn)(float, float) );

/** Compute step dt */
static __device__ void checkCflCondition(int offset, int iterations, float* localArray, GridDefinition* gridDefinition, Global* global);
...
/** Take snapshot */
static __device__ void takeSnapshot(int offset, int iterations, int* recordIndex, Snapshot* snapshots, Grid* grid, Model* model, float time);
...
void allocGlobalDevice(Global* global, int blocks, int iterations){
    HANDLE_CUDA(cudaMalloc(&global->reductionArray, blocks * sizeof(float)));
...
size_t requiredSharedMemory(){
    size_t sharedMemoryForReduction = sizeof(float) * THREADS_PER_BLOCK;
...
__global__ void gbeesKernel(int iterations, Model model, Global global, Snapshot* snapshots){
    ...
    __shared__ float localArray[THREADS_PER_BLOCK];
    ...
    for(int nm=0; nm<model.numMeasurements; nm++){
       ...
       if(model.performOutput){
        ...
        float mt = 0.0; // time propagated from the last measurement            
        float rt;        
        // slight variation w.r.t to original implementation as record time is recalculated for each measurement
        float recordTime = measurement->T / (model.numDistRecorded-1);
...
/** Calculate gaussian probability at state x given mean and covariance (used for first measurement) */
static __device__ float gaussProbability(int32_t* key, GridDefinition* gridDefinition, Measurement* measurement){    
    float mInvX[DIM];
    float diff[DIM];
    ...
    multiplyMatrixVector( (float*)measurement->covInv, diff, mInvX, DIM);
    float dotProduct = computeDotProduct(diff, mInvX, DIM);
...
static __device__ float gaussProbability(float* y, Measurement* measurement){    
    ...
    multiplyMatrixVector( (float*)measurement->covInv, diff, mInvX, measurement->dim);
    float dotProduct = computeDotProduct(diff, mInvX, measurement->dim);
    return exp(-0.5 * dotProduct);
}
...
/** Initialize boundary value */
static __device__ void initializeBoundary(Cell* cell, Model* model){
    float j = (*model->callbacks->j)(cell->x);
    cell->bound_val = j;
}

static __device__ void initializeGridBoundary(int offset, int iterations, float* localArray, GridDefinition* gridDefinition, Global* global){
    float boundaryValue = -DBL_MAX;    
    for(int iter=0; iter<iterations; iter++){        
...
static __device__ void pruneGrid(int offset, int iterations, GridDefinition* gridDefinition, Grid* grid, Global* global){    
    ...
    __shared__ uint32_t buffers[THREADS_PER_BLOCK * 2]; // shared memory for scan (float buffer)
    ...
    // initialize float buffer used list
    for(int iter=0; iter<iterations;iter++) {
        uint32_t usedIndex = getIndex(offset, iter); // index in the used list  
...
static __device__ void updateCtu(Cell* cell, Grid* grid, GridDefinition* gridDefinition) {         
   ...
   for(int i=0; i<DIM; i++){
        ...
        float flux = gridDefinition->dt*(cell->prob - iCell->prob) / (2.0 * gridDefinition->dx[i]);                    
        float vUpstream = iCell->v[i];
...
static __device__ void applyMeasurementCell(Cell* cell, Measurement* measurement, GridDefinition* gridDefinition, Grid* grid, Model* model){
    ...
    float prob = gaussProbability(y, measurement);   
    cell->prob *= prob;
}
